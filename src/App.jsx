...
function GroupManager({ groups, onGroupsChange }) {
  ...
  const createOption = async () => {
    ...
    try {
      const response = await fetch(
        `/api/groups/${selectedGroupForOption}/options`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        data.map(async (entry) => {
          try {
            const selectionsResponse = await fetch(
              `/api/mood/${entry.id}/selections`
            );
            const selections = await selectionsResponse.json();
            return { ...entry, selections };
          } catch (error) {
            console.error(
              `Failed to load selections for entry ${entry.id}:`,
              error
            );
            return { ...entry, selections: [] };
          }
    ...
    <>
      <div style={{ marginBottom: "2rem" }}>
        ...
        >
          {currentView === "history"
            ...
            : currentView === "entry"
            ? `Recording for ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString(
                [],
      ...
      {currentView === "history" ? (
        <>
          ...
          <div style={{ textAlign: "left", marginTop: "2rem" }}>
            {pastEntries.length === 0 ? (
              <p style={{ textAlign: "center", color: "#4C566A" }}>
        ...
        <>
          ...
          {/* Group Management Section for Entry */}
          <div style={{ marginBottom: "2rem" }}>
            <GroupManager groups={groups} onGroupsChange={loadGroups} />
          </div>
                      ...
                      days.push(
                        <div
                          ...
                          style={{
                            ...
                            borderRadius: "8px",
                            background: entry
                              ? getMoodIcon(entry.mood).color + "20"
                              : "transparent",
                            border:
                            ...
                            color: entry
                              ? getMoodIcon(entry.mood).color
                              : "#666",
                          }}
                        >
                          {entry
                            ? (() => {
                                const { icon: IconComponent } = getMoodIcon(
                                  entry.mood
                                );
