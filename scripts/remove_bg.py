...
def parse_args(argv):
    ...
    while i < len(argv):
        ...
        if a in ("-i", "--input") and i + 1 < len(argv):
            ...
            args["tolerance"] = int(argv[i + 1])
            i += 2
        elif a == "--hue-min" and i + 1 < len(argv):
            args["hue_min"] = float(argv[i + 1])
            i += 2
        elif a == "--hue-max" and i + 1 < len(argv):
            args["hue_max"] = float(argv[i + 1])
            i += 2
        elif a == "--sat-min" and i + 1 < len(argv):
            args["sat_min"] = float(argv[i + 1])
            i += 2
        elif a == "--val-min" and i + 1 < len(argv):
            args["val_min"] = float(argv[i + 1])
            i += 2
        elif a == "--feather" and i + 1 < len(argv):
            args["feather"] = float(argv[i + 1])
            i += 2
        elif a == "--max-dim" and i + 1 < len(argv):
            args["max_dim"] = int(argv[i + 1])
            i += 2
...
def remove_bg_rgb(
    im: Image.Image, tolerance: int = 40, target=(0, 128, 0)
) -> Image.Image:
    im = im.convert("RGBA")
