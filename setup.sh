...
if lsof -i :5173 >/dev/null 2>&1; then
    echo "⚠️  Port 5173 is already in use. Please stop the service using it or edit docker-compose.yml"
    exit 1
fi

if lsof -i :5000 >/dev/null 2>&1; then
    echo "⚠️  Port 5000 is already in use. Please stop the service using it or edit docker-compose.yml"
    exit 1
fi
